...
#endif
#include "string.h"
#include "app/tlkapp_general/bp_uart.h"
volatile uint16 g_connHandle = 0;
...
static const gap_periConnectParams_t my_periConnParameters = {6, 15, 0, 1000};
...
/*****************************SPP printer*********************************/
//const u8 telinkSPPServiceUUID[16] = TELINK_SPP_SERVICE_UUID;
//const u8 telinkSPPRxUUID[16] = TELINK_SPP_RX_UUID;
//const u8 telinkSPPTxUUID[16] = TELINK_SPP_TX_UUID;
...
static uint08 TelinkSppServiceUUID[16];
static uint08 TelinkSppDataServer2ClientUUID[16];
static uint08 TelinkSppDataClient2ServerUUID[16];
...
//SPP data descriptor
static const uint08 TelinkSPPS2CDescriptor[] 		 		= "Module->Phone";
static const uint08 TelinkSPPC2SDescriptor[]        		= "Phone->Module";
...
//spp attributes
//u8 propWrite = CHAR_PROP_WRITE;
//u8 propNotify = CHAR_PROP_NOTIFY;
//u16 sppTxCCC = 0;
#if TLKMDI_LEOTA_ENABLE
...
//	uart_send(UART1, &p->value,len);
//	uart_send(UART1, "ACK",3);
	for(uint16 i = 0; i < len; i++) {
		if(!rb_is_full(&uart_rb_rx))
		{
		        rb_push(&uart_rb_rx, (&p->value)[i]);
		}
		else
		{
...
//	{0,ATT_PERMISSIONS_READ,2,sizeof(TelinkSPPC2SDescriptor),(uint08*)&userdesc_UUID,(uint08*)(&TelinkSPPC2SDescriptor)},
	...
	//spp
//		{TELINK_SPP_SERVICE_HANDLE, ATT_PERMISSIONS_READ, 2, 16, (uint08*)&my_primaryServiceUUID, (uint08*)telinkSPPServiceUUID},
//		{TELINK_SPP_RX_CHAR_HANDLE, ATT_PERMISSIONS_READ, 2, 1, (uint08*)&my_characterUUID, (uint08*)&propWrite},
//		{TELINK_SPP_RX_VALUE_HANDLE, ATT_PERMISSIONS_WRITE, 16, 20, (uint08*)telinkSPPRxUUID, 0, &spp_onReceiveData},
//		{TELINK_SPP_TX_CHAR_HANDLE, ATT_PERMISSIONS_READ, 2, 1, (uint08*)&my_characterUUID, (uint08*)&propNotify},
//		{TELINK_SPP_TX_VALUE_HANDLE, ATT_PERMISSIONS_RDWR, 16, 20, (uint08*)telinkSPPTxUUID, 0, 0},
//		{TELINK_SPP_TX_CCC_HANDLE, ATT_PERMISSIONS_READ | ATT_PERMISSIONS_WRITE, 2, 2, (uint08*)&clientCharacterCfgUUID, (uint08*)&sppTxCCC, 0},
...
//        uint08 default_notify_uuid[16] = {0x11,0x2B,0x0d,0x0c,0x0b,0x0a,0x09,0x08,0x07,0x06,0x05,0x04,0x03,0x02,0x01,0x00};
//        uint08 default_write_uuid[16] = {0x10,0x2B,0x0d,0x0c,0x0b,0x0a,0x09,0x08,0x07,0x06,0x05,0x04,0x03,0x02,0x01,0x00};
//
//        memcpy(TelinkSppServiceUUID, default_service_uuid, 16);
//        memcpy(TelinkSppDataServer2ClientUUID, default_write_uuid, 16);
//        memcpy(TelinkSppDataClient2ServerUUID, default_notify_uuid, 16);
//	}
//	else{
		memcpy(TelinkSppServiceUUID,fc_hw_flag.service_uuid,16);
		memcpy(TelinkSppDataServer2ClientUUID,fc_hw_flag.characteristic_notify_uuid,16);
		memcpy(TelinkSppDataClient2ServerUUID,fc_hw_flag.characteristic_write_uuid,16);
//	}
