...
void process_at_cmd(uint08 * cmd) {
	if (cmd[0] == 'A' && cmd[1] == 'T') {
		...
		{
			chr_buff = strchr(cmd, '=');
			if (!chr_buff) {notify_cmd_error(); return; }
			chr_buff++;
			...
			if (name_len > 0) {
				...
				uart_send(UART1, (uint08*)"ERROR: Empty name\r\n", 19);
			}
		}
		...
		else if ((strstr((const char * ) cmd, "+MAC=") != NULL)) //BTADDRESS
		{
			...
			if (tlkmmi_btmgr_setAddr(bt_addr) == TLK_ENONE) {
				...
				uart_send(UART1, (uint08*)"ERROR: Failed to set BT address\r\n", 30);
			}
		}
...
{
	if (rb_is_empty(&uart_rb_rx)) return;
	uint08 p_data;
	if (fc_hw_flag.hw_flow_ctrl_flg) { //HW FLOW
		uart_set_cts_en(UART1);
		while (!rb_is_empty(&uart_rb_rx)) {
			if (rb_pop(&uart_rb_rx, &p_data)) {
				uart_send_byte(UART1, p_data);
			}
		}
		if(rb_is_empty(&uart_rb_rx))uart_set_cts_dis(UART1); //No flow control
	} else {
		while (!rb_is_empty(&uart_rb_rx)) {
			if (rb_pop(&uart_rb_rx, &p_data)) {
				uart_send_byte(UART1, p_data);
	...
	if (rb_count(&uart_rb_rx) < RX_TX_FIFO_SIZE / 2 && btc_conn_flag) {
