...
volatile uint08 uart_busy_flag = 0; //0 none, 1 spp , 2, ble
volatile uint32_t last_sent_tick = 0;   // Last time SPP sent data (in ms)
#define DATA_DELAY_SWITCH 1000000
...
static void send_data_to_uart(ringbuffer_t *data_buffer, bool is_spp)
{
	...
	if (fc_hw_flag.hw_flow_ctrl_flg) { //HW FLOW
		...
		if (rb_count(&spp_rb_rx_1) < RX_TX_FIFO_SIZE / 2 && is_spp) {
...
void process_bt_handler()
{
    ...
    if(uart_busy_flag == 0) {
        ...
        else if(blec_conn_flag && !rb_is_empty(&ble_rb_rx))uart_busy_flag = 2;
    }

    if (uart_busy_flag == 1) {
        send_data_to_uart(&spp_rb_rx_1, true);  // SPP
        if (rb_is_empty(&spp_rb_rx_1) && clock_time_exceed(last_sent_tick, DATA_DELAY_SWITCH)) {
            uart_busy_flag = 0;
        }
    } else if (uart_busy_flag == 2) {
